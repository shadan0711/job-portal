...
import { 
  ArrowRight, Wrench, Zap, Wind, HardHat, Hammer, Shield, Layers 
} from 'lucide-react';
...
const Home = () => {
  ...
  const categories = [
    { id: 1, name: "Electrician", icon: Zap, count: "10 Vacancies" },
    { id: 2, name: "Plumber", icon: Wrench, count: "10 Vacancies" },
    { id: 3, name: "AC Technician", icon: Wind, count: "10 Vacancies" },
    { id: 4, name: "aaa", icon: HardHat, count: "10 Vacancies" },
    { id: 5, name: "bbbb", icon: Hammer, count: "10 Vacancies" },
    { id: 6, name: "cccc", icon: Shield, count: "10 Vacancies" },
    { id: 7, name: "ddd", icon: Layers, count: "10 Vacancies" },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white selection:bg-cyan-500 overflow-x-hidden">
      ...
      {/* --- CATEGORY SECTION (Click leads to new page) --- */}
      <section className="max-w-7xl mx-auto pt-20 pb-12 px-6">
        ...
        {/* Responsive Grid: Mobile = 2, Tablet = 3, Laptop = 5 */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
          {categories.map((cat) => {
            const IconComponent = cat.icon;

            return (
              <Link to={`/jobs/${cat.name}`} key={cat.id}>
                <motion.div
                ...
                >
                  ...
                  <h3 className="font-bold text-sm text-white tracking-wide group-hover:text-cyan-400 transition-colors">
                    {cat.name}
                  </h3>
...
//             Select a category to view all specialized job opportunities
//           </p>
//         </div>
